...
const ProjectsTable = ({data, setProjectId, setShowSidebar}: {
    ...
    const header = ['Asana ID', 'Название проекта', 'Владелец', 'Количество исполнителей', 'Статус проекта']
    ...
    return (
        <table className="min-w-full text-center">
            ...
            <tbody>
            {data?.length !== 0 ? (data?.map(project => <tr key={project.id}
                    ...
                    <td className="whitespace-nowrap border-r dark:border-neutral-500 px-[5px] py-[20px]">{project.manager.map(manager =>
                        <span key={manager.id}>{manager.last_name} {manager.first_name}</span>)}</td>
                    <td className="whitespace-nowrap border-r dark:border-neutral-500 px-[5px] py-[20px] cursor-pointer"
                        onClick={() => navigation(`/dashboard/projects/${project.id}/assessors`)}>{project.assessors_count}</td>
