...
const PersonalAssessorInfoTable = ({assessorId}: {assessorId: string | number | undefined }) => {
    ...
    const patchAssessorInfo = useMutation(['currentAssessorInfo'], ({id, data}: any) => AssessorService.patchAssessor(id, data),{
        onError: () => {
            errorNotification('Ошибка при обновлении информации')
            queryClient.invalidateQueries('currentAssessorInfo')
        },
        onSuccess: () =>{
        ...
        <div className="z-0 rounded-[20px] overflow-hidden">
            <table className="w-full">
                ...
                <tr className='bg-white'>
                    ...
                    <td className="whitespace-nowrap border-r dark:border-neutral-500 py-[5px]">
                        <input disabled={isDisabled} className="w-full text-center bg-white disabled:border-none disabled:opacity-50" {...register('email')}/>
                    </td>
                    <td className="whitespace-nowrap border-r dark:border-neutral-500 py-[5px]">
