...
const TableLog = () => {
    ...
    return (
        <div className="rounded-t-[20px] border border-b-gray-400 bg-white">
                ...
                {table.getHeaderGroups().map((headerGroup) => (
                    <tr key={headerGroup.id}>
                        {headerGroup.headers.map((header) => (
                            <th key={header.id} className="bg-[#E7EAFF]">
            ...
            <button onClick={() => store.setEditableLoginAndPassword(!store.isEditableLoginAndPassword)}>{store.isEditableLoginAndPassword? "Сохранить": "Редактировать"}</button>
        </div>
    );
};
