...
export default class AssessorService{
    static fetchAssessors = (page: number | string,projectId: any, ordering: string = '') => $api.get<AssessorResponse>(`/api/projects/${projectId}/assessors/?page=${page}&page_size=10&ordering=${ordering}`).then(res => {
        return res.data
    })
